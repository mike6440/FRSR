...
#define VERSION		"4"//"3"
#define EDITDATE	"170110"//"161222"
const byte	EEPROM_ID = 4;
//v2 - add adc stuff in action
//v3 - Nsweep in eeprom
//v4 - Nchans=7, int adcmv[NCHANS*NSWEEP] shoehorn in full sweep.
//		changed ADS1115_CONVERSIONDELAY to 2 to speed up adc for sweep
...
const unsigned int	NSWEEP = 20;
const unsigned int NCHANS = 7;
// 250*7=1750 samples in 3 sec = 583 samples/sec (583 Hz)
...
// READING THE ADC
int Chan[]={2,4,5,6,7,8,11};
// int Chan[]={0,1,2,3,7,8,11};
...
void		GetAdc (int16_t *, unsigned int );   // ads adc
double GetAdcVolts(unsigned int ch);	 // ads adc
...
double		ThermistorTemp(double v,double Rref);
...
void	Action(char *cmd)
{
  ...
  // READ ADC CHANNEL
	...
	if(strlen(cmd)==2){ix = cmd[1] - 48;}
		...
		Serial.println("Mvolts  adc"); 
			while ( !Serial.available()	 ) {
				ddum=GetAdcVolts(ix);
				GetAdc(&i16,ix);
				Serial.print(ddum, 0);Serial.print(" ");Serial.println(i16);
				delay(100);
			}
		}
  }
...
// 		for(iy=0; iy<4; iy++){
// 			for(iz=0; iz<4; iz++){
				GetAdc( (adcmv+isamp), Chan[iz] );
				//ddum=GetAdcVolts(iz);				//testing
// 				if(1){
// 					Serial.print(iy); Serial.print(" "); Serial.print(iz); Serial.print(" "); 
// 					Serial.print(isamp); Serial.print(" "); 
// 					  Serial.print(Chan[iz]); Serial.print(" "); 
// 					Serial.print( adcmv[isamp] ); Serial.print("    ");
// 				}
				isamp++;
			}
// 			Serial.println("");
  ...
  // 5REF	ON/OFF
	...
	// THERMISTER
		...
		//T1
		ddum=GetAdcVolts(0);
		Serial.print("v1="); Serial.println(ddum,2);
		fdum=ThermistorTemp(ddum, ee.Rref[0]);
		Serial.print("T1="); Serial.println(fdum,2);
		//T2
		ddum=GetAdcVolts(1);
		Serial.print("v2="); Serial.println(ddum,2);
		fdum=ThermistorTemp(ddum, ee.Rref[0]);
		Serial.print("T1="); Serial.println(fdum,2);
	}
...
// 
  ...
  Serial.println("");
  Serial.print(" Rref = ");
  Serial.print(ee.Rref[0],1); Serial.print("  "); Serial.print(ee.Rref[1],1);
  Serial.println("");
...
//======================================================================================
void GetAdc (int16_t *v, unsigned int chan) {
  //GetAdc returns a single read of 16 bit adc channel chan.
//   Serial.print("     chan=");Serial.print(chan,DEC);
  int16_t adc, adcx;
  if (chan >= 0 && chan <= 3){
// 		adcx = ads0.readADC_SingleEnded(chan);
// 		adcx = ads0.readADC_SingleEnded(chan);
// 		adcx = ads0.readADC_SingleEnded(chan);
// 		adcx = ads0.readADC_SingleEnded(chan);
...
//======================================================================================
double GetAdcVolts(unsigned int ch) {
...
// 	if(v>threshold[1]) return 200;
	...
	r = Rref * (v / (vx-v));
	Serial.print("rtherm="); Serial.println(r,2);
...
// 	TempHead = 1.0 / d - 273.15;
	...
	Serial.print("temp="); Serial.println(t,3);
