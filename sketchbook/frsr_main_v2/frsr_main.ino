...
#define VERSION     "2"// Originally copied from rosr v29
#define EDITDATE    "161217"
...
const unsigned long default_Nsamps = 250;
...
struct eeprom {
  byte id;
  unsigned long Nsamps;  // total samps over the hemisphere.
...
void    Action(char *cmd)
{
  ...
  //Output means and stdevs. Get the time for the full process.
    ...
    //clear register
		...
		sweeptime=millis();
    //Loop 
    for( isamp=0; isamp<10; isamp++){ //ee.Nsamps  testing
    	// Read all adc chans
			for (ix = 0; ix < ADCMAX; ix++) {
				ddum=GetAdcVolts(ix);
				vmean[ix] += ddum;
    ...
    //void    MeanStdev(double *sum, double *sum2, int N, double missing)
		...
		//Print results
  	//header
    Serial.print("Read ");Serial.print(ee.Nsamps,DEC); Serial.println(" scans.");
