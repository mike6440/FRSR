//NOTE:
// to do
//    micros() overflow issue. millis() too.
// go to !!!! to pick up where you left off.

#define PROGRAMNAME "frsr_main"
#define VERSION		"08"
#define EDITDATE	"20170324T145156Z"
#define	EEPROM_ID  7
...
#define	NOutStr 450  	// size of data storage area
...
void	Action(char *cmd)
{
	...
	Serial.println("an -- ADC10 Chan n(0-6)       A  -- ADC16 Chan n(0-3)");
  ...
  // READ ADC10 CHANNEL
	...
	// ADC LOOP
		...
		NadirCheck();
		Serial.print("N ");Serial.print(NadirTimeUsecs);Serial.print(", ");Serial.println(CycleUsecs);
		SampUsecs=(unsigned long) (CycleUsecs / NSWEEP / 2);
		horiz1usec=NadirTimeUsecs+CycleUsecs/4;
		horiz2usec=horiz1usec+CycleUsecs/2;
		...
		// Wait for horizon 1
		while( micros() < horiz1usec ); // wait for the time
		Serial.print("S");Serial.println(micros()-NadirTimeUsecs);
	...
	// TIME
		...
		Serial.println("floatToString(s, 123.456, 2, 8);  ");
		floatToString(buf, 123.456, 2, 8);
		Serial.println(buf);
	}
	// EEPROM
...
**********************************/
{
	...
	****************/	
	...
	Serial.print("Rad(");Serial.print(ee.shadowchannel-1);
	Serial.print(") "); Serial.print(adcx,1);
	Serial.print("/"); Serial.print(ee.Low);
	...
	**********************/
	if( MotorPwrFlag == ON) {
		...
		***********************/ 
		...
		if ( CycleUsecs > 8000000 ){Serial.print("x5 ");CycleUsecs=6500000;}
		return OK;
	} 
	return OK;
}
...
// 	// MFR Temp
// 	floatToString(bufx, (double)TempHead, 1, 4);
// 	strcat(OutStr, bufx);
// 	strcat(OutStr, ",\0");
// 	df_index += 5;
// 	Serial.print("  ");Serial.print(TempHead,2);
// 	Serial.print("--");Serial.print(bufx);
// 	
